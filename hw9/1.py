...
# step 1
    ...
    D[1:-1, 1:-1, 1:-1] = u[1:-1, 1:-1, 1:-1]/dt + 0.5 * (u[2:, 1:-1, 1:-1] - 2*u[1:-1,
    1:-1, 1:-1] + u[:-2, 1:-1, 1:-1])/h**2 + (u[1:-1, 2:, 1:-1] - 2*u[1:-1, 1:-1, 1:-1] +
    u[1:-1, :-2, 1:-1])/h**2 + (u[1:-1, 1:-1, 2:] - 2*u[1:-1, 1:-1, 1:-1] + u[1:-1, 1:-1, :-
    2])/h**2
    beta[1, :, :] = u[0, :, :]
...
# step 2
    ...
    alpha = np.zeros_like(u)
    beta = np.zeros_like(u)
    D[1:-1, 1:-1, 1:-1] = u_1[1:-1, 1:-1, 1:-1]/dt - 0.5 * (u[1:-1, 2:, 1:-1] - 2*u[1:-1,
1:-1, 1:-1] + u[1:-1, :-2, 1:-1])/h**2
    beta[:, 1, :] = u[:, 0, :]
...
# step 3
    ...
    alpha = np.zeros_like(u)
    beta = np.zeros_like(u)
    D[1:-1, 1:-1, 1:-1] = u_2[1:-1, 1:-1, 1:-1]/dt - 0.5 * (u[1:-1, 1:-1, 2:] - 2*u[1:-1,
    1:-1, 1:-1] + u[1:-1, 1:-1, :-2])/h**2
    beta[:, :, 1] = u[:, :, 0]
    for i in range(1, k-1):
        alpha[:, :, i+1] = -np.full(k, A) / (np.full(k, B) + C*alpha[:, :, i])
        beta[:, :, i+1] = (D[:, :, i] - C*beta[:, :, i]) / (np.full(k, B) + C*alpha[:, :,
i])
